...
|---|---|
| `CEMACON SA` | legal name, uppercase (e.g. `EXAMPLE COMPANY SRL`) |
| `Cemacon` | commercial brand |
| `677858` | fiscal code (CUI), no `RO` prefix |
| `https://cemacon.ro` | `https://www.example.com` |
| `https://cemacon.ro/cariere/` | the open-positions listing page |
| `` | the job sitemap URL (or `""` if the site has none) |
| `https://cemacon.ro/cariere/` | canonical job-permalink prefix, e.g. `https://www.example.com/jobs/` |
| `Cluj-Napoca` | HQ city (falls back to `România` in the transform) |
| `article.cemacon-cariere-card` / `.cemacon-cariere-card__title a` / `.cemacon-cariere-card__excerpt` | primary CSS selectors for the listing (keep the generic fallbacks after them) |
| `TheTatu13` / `cemacon-sa-nodejs-scraper` | the derived repo's owner / name |
